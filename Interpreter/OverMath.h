...
#ifndef INTERPRETER_OVERMATH_H
#define INTERPRETER_OVERMATH_H
...
namespace overmath {
    ...
    struct function {
        ...
        boost::optional<const parameter&> find_parameter(const wstring& name) const {
            auto found = find_if(params.begin(), params.end(),
            [&](const parameter& p){
               return p.name == name;
            });
